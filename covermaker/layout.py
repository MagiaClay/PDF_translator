...
class Line(object):
    '''行。每行保存了很多词。包含word，字体，行间距，字体数据等'''
    ...
    def _update(self, word, sign):
        ...
        left, top, right, bottom = self._font.getbbox(word)
        text_width = right - left  # 对应getsize【0】
        text_height = bottom - top  # 对应getsize[1]
        if self._section.dir == 'h':

            self._words_width += sign * text_width
        else:
            self._words_width += sign * text_height
...
class Layout(object):
    ...
    def update(self, text_box, dir, valign, halign):
        ...
        self._dir = dir
        if dir == 'h':
            self._update_h(text_box, valign, halign)  # 如果识别是水平文本
        else:
            ...
            for i, line in enumerate(self.lines):
                ...
                for word in line.words:
                    for c in word:
                        ...
                        fw = text_width
                        fh = text_height
                        yield c, (x, pos[1]), 0, (fw, fh)  # 带yield的函数是一个郑承气，而不是一个函数了，返回
                        # 这个生成器只用调用迭代时候才开始执行文件中的内容，遇到yied后执行return，返回后如果再next则从yied后凯斯执行
...
def _build_lines(text, font, words, boxw, boxh, font_size, lespc, lispc, section):
    ...
    lines = []
    prei, i = 0, 0
    line = Line(font, lespc, section)  # 判断==生成一个Liie对象
    while i < len(words):
        word = words[i]
        line.append(word)
        lw = line.get_display_width()  # 获取当前word长度
...
def _build_max_font_lines(text, section):
    ...
    while lfs <= rfs:
        ...
        font = _get_font(section.font, mfs) # 此处以及自适应字体大小
        #
        lines = _build_lines(text, font, words, boxw, boxh, mfs, lespc, lispc, section)  # 此处完成line的分行
        if mfs == lfs:
            break
        if lines:
            lfs = mfs
...
def layout_text(text, section) -> Layout:
    ...
    lines, font, font_size, line_spacing, letter_spacing = _build_max_font_lines(
        text, section)  # 在文本框内寻找能最大利用文本框显示区域字号，并执行分行操作line[]即size+分行line
    # print(font_size)
    # 进行布局运算
    ret = Layout(lines, font, font_size, line_spacing, letter_spacing)  # 打包输出
    # ret = Layout(lines, font, 40, line_spacing, letter_spacing)  # 打包输出
